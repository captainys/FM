...
2025/11/27 どうもときどきゴミデータがFM-7側から送られてしまってサーバーが反応しない状態になるようなので、コマンドが1秒以内に完了しない場合は、タイムアウトにしてコマンドバッファをクリアすることにした。FM-7側から、PRINT #1,"REQ0"; みたいな感じでコマンドを送ると、サーバーは、バイナリローダーを送り返すようにした。なお、セミコロンは必須。
...
(使い方その2)
FM-7上でタイプするプログラムはやや長くなりますが、サーバー側は一度起動してしまったら、あとは触らなくてもFM-7側だけでリダイレクタをインストールすることができるようにしました。
...
サーバーを起動したら、このプログラムをタイプして打ち込むだけです。なお、30行の最後のセミコロンは絶対必要です。例えば、実機を展示して、サーバー側はロックして放っておきたいような場合はこの方法が有効だと思います。
...
(非公式対応)というのはDisk I/Oを直接アクセスしないように書き換えることで起動可能。
  ...
  F-BASIC 3.0 [実機FM-7,実機FM77AV40]
    ...
    2025/11/14 Disk BASIC 3.0 Level 10は起動中使用しないコードをFC00～FC7Fの領域に
               転送するため、リダイレクタを破壊して起動に失敗していたことが判明した
               が、転送しているコードは未使用なので、この部分を検出してNOP化すること
               で普通に起動できるようになった。
